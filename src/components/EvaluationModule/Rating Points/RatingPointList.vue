        ...
        <div class="app-page-title">
          <div class="page-title-wrapper">
            <div class="page-title-heading">
              ...
              <div>
                   {{ $store.getters.GET_RATING+ ' '+$store.getters.GET_POINT+' '+$store.getters.GET_LIST}}
              </div>
              <router-link
                :to="{ path: '/create-rating-point' }"
                v-if="hasPermission('add_ratingpoint')"
              >
                ...
                <table class="table table-hover">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">  {{$store.getters.GET_RATING+ ' '+$store.getters.GET_POINT+' '+ $store.getters.GET_NAME}}</th>
                      <th scope="col"> {{$store.getters.GET_RATING+ ' '+$store.getters.GET_POINT+' '+ $store.getters.GET_VALUE}}</th>
                      <th scope="col"> {{ $store.getters.GET_ACTION }}</th>
                    </tr>
                  </thead>
                  <tbody v-if="all_rating_points && all_rating_points.length > 0">
                    <tr v-for="(rating, index) in all_rating_points" :key="index">
                      <td>{{ index + 1 }}</td>
                      <td>{{ rating.name }}</td>
                      <td>
                        <span class="badge badge-pill badge-info baseColor">{{ rating.value }}</span>
                      </td>
                      ...
                      <td class="td_action">
                        <router-link
                          ...
                          :to="{
                            ...
                            params: { id: rating.id },}"
                          v-if="hasPermission('change_ratingpoint')"
                        >
                        ...
                        <i
                          ...
                          v-if="hasPermission('delete_ratingpoint')"
                        ></i>
                      </td>
                    </tr>
                  </tbody>
